...
//   if (body.action === 'build_full_sequence') return json(buildFullSequence(body.brief, body.copy));
//
// ─────────────────────────────────────────────────────────────────────────────
...
// ── Master wireframe — all 13 emails, 4 sequences ────────────────────────────
// day: relative to each sequence's trigger event (not absolute campaign days)

var _SB_WIREFRAME = [
  // SEQ-1 — Welcome (trigger: waitlist_signup)
  {seq:'SEQ-1',num:1,global:1,  day:0,  role:'Hook — You are in — deliver the promise, make them glad they joined',              trigger:'waitlist_signup',  theme:'welcome',      stage:'hook'},
  {seq:'SEQ-1',num:2,global:2,  day:2,  role:'Problem deepener — name the exact 6:30 PM pain, agitate before solving',           trigger:'waitlist_signup',  theme:'problem',      stage:'problem'},
  {seq:'SEQ-1',num:3,global:3,  day:5,  role:'Social proof + founding price — one story, one stat, founding urgency',            trigger:'waitlist_signup',  theme:'proof_intro',  stage:'solve'},
  // SEQ-2 — Nurture (trigger: seq1_complete)
  {seq:'SEQ-2',num:1,global:4,  day:7,  role:'Savings spotlight — $1,336/year — show the annual maths clearly',                  trigger:'seq1_complete',    theme:'savings',      stage:'value'},
  {seq:'SEQ-2',num:2,global:5,  day:10, role:'Speed spotlight — 30 minutes fridge to table — paint the scene',                   trigger:'seq1_complete',    theme:'speed',        stage:'value'},
  {seq:'SEQ-2',num:3,global:6,  day:14, role:'Waste spotlight — 69.5% less food waste — make the number feel real',              trigger:'seq1_complete',    theme:'waste',        stage:'value'},
  {seq:'SEQ-2',num:4,global:7,  day:18, role:'Objection handler — Is $7.99/month worth it? Answer directly and honestly',        trigger:'seq1_complete',    theme:'objection',    stage:'agitate'},
  {seq:'SEQ-2',num:5,global:8,  day:25, role:'Final nurture proof — founding price window closing, strongest proof statement',   trigger:'seq1_complete',    theme:'proof_final',  stage:'proof'},
  // SEQ-3 — Urgency (trigger: june_17_urgency — June 17, 14 days before July 1 launch)
  {seq:'SEQ-3',num:1,global:9,  day:0,  role:'Countdown begins — 14 days to launch, founding price window framing',              trigger:'june_17_urgency',  theme:'urgency',      stage:'cta'},
  {seq:'SEQ-3',num:2,global:10, day:4,  role:'Founding price closes in 10 days — financial specificity, loss framing',           trigger:'june_17_urgency',  theme:'price',        stage:'cta'},
  {seq:'SEQ-3',num:3,global:11, day:10, role:'Last 4 days — what changes on July 1, concrete and specific',                      trigger:'june_17_urgency',  theme:'urgency_final',stage:'cta'},
  {seq:'SEQ-3',num:4,global:12, day:13, role:'Last chance — tomorrow is July 1, no fake scarcity, just honest final call',       trigger:'june_17_urgency',  theme:'last_chance',  stage:'cta'},
  // SEQ-4 — Launch Day (trigger: july_1_launch — July 1)
  {seq:'SEQ-4',num:1,global:13, day:0,  role:'App is live — download now — celebrate with them, lead with the win',              trigger:'july_1_launch',    theme:'launch',       stage:'hook'}
];

// Day offsets for the calendar merger (SEQ-1/2 are relative; SEQ-3/4 are anchored)
var _SB_SEQ_OFFSETS = { 'SEQ-1':0, 'SEQ-2':0, 'SEQ-3':28, 'SEQ-4':42 };
 ...
 *
 * brief.email_sequences (string or number):
 *   'seq1_only' | 1  → SEQ-1 (3 emails)
 *   'seq1_seq2' | 2  → SEQ-1 + SEQ-2 (8 emails)   [default]
 *   'seq1_seq2_seq3' | 3 → SEQ-1–3 (12 emails)
 *   'full' | 4+      → SEQ-1–4 (13 emails)
 *
 * brief.email_variants (string or number):
 *   'variant_a' | 1  → Money angle only
 *   'variant_b'      → Simplicity angle only
 *   'both' | 2+      → Both A and B [default]
 *
 * Returns { ok:true, emails:[{seq_id, sequence_code, email_number, ...}] }
 */
function buildEmailCalendar(brief, copy) {
  try {
    ...
    // ── Normalize sequence / variant mode ──────────────────────────────────
    var seqRaw     = brief.email_sequences;
    var variantRaw = brief.email_variants;

    var seqMode = _sbNormalizeSeqMode(seqRaw);
    var varMode = _sbNormalizeVariantMode(variantRaw);

    var activeSeqs  = _SB_SEQ_MAP[seqMode] || _SB_SEQ_MAP['seq1_seq2'];
    var wireframe   = _SB_WIREFRAME.filter(function(e) {
    ...
    var systemPrompt =
      ...
      'Proof bar: ' + (copy && Array.isArray(copy.proof_bar) ? copy.proof_bar.join(' · ') : (copy && copy.proof_bar || '')) + '\n\n' +
      '=== ' + variantInstr + ' ===\n\n' +
      '=== 7-STEP FRAMEWORK ===\n' +
      'Hook → Problem → Agitate → Solve → Value → Proof → CTA\n' +
      'Every email focuses on its assigned stage (see wireframe). Do not try to hit all 7 in every email.\n\n' +
    ...
    var resp = UrlFetchApp.fetch('https://api.anthropic.com/v1/messages', {
      ...
      payload: JSON.stringify({
        ...
        messages: [{
          role:    'user',
          content: 'Generate the ' + wireframe.length + '-email sequence for the ' + (brief.icp || 'target') + ' ICP. Return only the JSON array.'
        }]
      }),
    ...
    // ── Supplement from wireframe + write to sheet ─────────────────────────
 ...
 * email sequence. Post count and frequency come from brief.post_count and
 * brief.post_frequency. Each post is thematically aligned with the nearest
 * email in the active sequence.
 *
...
function buildSocialCalendar(brief, copy) {
  try {
    ...
    var activeWF      = _SB_WIREFRAME.filter(function(e) { return activeSeqs.indexOf(e.seq) !== -1; });
    var themeSchedule = _sbBuildSocialSchedule(activeWF, postCount);
    ...
    var systemPrompt =
      ...
      'Each post is 1 day ahead of the aligned email so social primes the inbox open.\n' +
      scheduleDesc + '\n\n' +
 ...
 * SEQ-3 send_days are offset +28 days; SEQ-4 +42 days to place them on
 * the absolute campaign timeline (approx. June 17 and July 1 respectively).
 *
...
function _sbMergeCalendar(emails, posts) {
  ...
  emails.forEach(function(e) {
    var absDay = (e.send_day || 0) + (_SB_SEQ_OFFSETS[e.sequence_code] || 0);
...
/**
 * Looks up a wireframe entry by seq_id string (e.g. 'SEQ-2-E3').
 */
function _sbFindWireframe(seqId) {
  var m = String(seqId || '').match(/^(SEQ-\d+)-E(\d+)$/);
  if (!m) return null;
  var seq = m[1], num = parseInt(m[2], 10);
  for (var i = 0; i < _SB_WIREFRAME.length; i++) {
    if (_SB_WIREFRAME[i].seq === seq && _SB_WIREFRAME[i].num === num) return _SB_WIREFRAME[i];
  }
  return null;
}

/**
 * Normalizes email_sequences to a string mode key.
 * Accepts string mode names OR integer (1-4+).
 */
...
/**
 * Normalizes email_variants to 'variant_a', 'variant_b', or 'both'.
 * Accepts string mode names OR integer (1 = variant_a, 2+ = both).
 */
...
/**
 * Builds a theme schedule for social posts by spacing postCount items
 * evenly across the active email wireframe. Each post is scheduled 1
 * day before its aligned email to prime the audience.
 */
function _sbBuildSocialSchedule(wireframe, postCount) {
  ...
  for (var i = 0; i < postCount; i++) {
    ...
    var offset = _SB_SEQ_OFFSETS[item.seq] || 0;
...
/**
 * Run from Apps Script editor (Run → _testSequenceBuilder) to verify
 * API connectivity and schema resolution without writing to Sheets.
 */
function _testSequenceBuilder() {
  ...
  Logger.log('=== WIREFRAME (3 emails):');
  var wf = _SB_WIREFRAME.filter(function(e){ return e.seq==='SEQ-1'; });
  wf.forEach(function(e){ Logger.log('  ' + e.seq + '-E' + e.num + ' | Day ' + e.day + ' | ' + e.theme); });

  Logger.log('=== Calling buildEmailCalendar (seq1_only / variant_a)...');

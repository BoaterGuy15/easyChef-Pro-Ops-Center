...
- **Use Liquid custom properties with default filter in Klaviyo templates** — Klaviyo's template validator rejects custom properties (person.meals_cooked, person.pantry_items, person.alpha_number, etc.) even with correct default filter syntax. Standard properties (person.first_name) work with 1-arg: `{{ person.first_name | default: 'there' }}`. Custom properties must use static text or `{% if %}` blocks — NOT `{{ person.custom_prop | default: X }}`. Subject lines: plain text only — NO Liquid in flow step subject fields. Alpha templates E1-E6 are fully static (no person.X) until Firebase personalization wired post-July 1.
